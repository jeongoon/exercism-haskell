 module IORef (Deque, mkDeque, pop, push, shift, unshift) where
  ...
  ( Kleisli readIORef >>>
    ( Kleisli (maybe (return Nothing) (fmap Just . shiftLastV dq))
      ||| ( first (Kleisli (shiftVPlus _nextMaybeNodeR))
            >>> arr flipSome
            >>> second ( cleanDequeData >>> Kleisli (writeIORef dq) )
            >>> arr (Just . fst)
          )
      )
  ) dq

  where flipSome = \((mbf, v), b) -> (v, (fromJust mbf,b)) -- f: front, b: back
...
pop dq =
  ...
  where flipSome = \(f, (mbb, v)) -> (v, (f, fromJust mbb))
...
cleanDequeData
  :: Arrow arw => arw (NodeRPair v) (DequeInner v)
cleanDequeData = arr $
  \orig@(l, r) -> if l == r
                  then Left  (Just l)
                  else Right orig
